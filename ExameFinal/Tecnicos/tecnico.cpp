...
Tecnico CriarTecnico() {
    ...
    // Aloca memória para o nome do técnico
...
void AdicionarTrabalhos(int Id) {
    ...
    while (fgets(linha, sizeof(linha), f) && total < 200) {
        // Lê campos da linha garantindo terminação \0
        if (sscanf(linha, "%d;%49[^;];%49[^;];%d;%49[^;];",
            ...
            // Incrementa trabalhos se for o técnico certo
            if (tecnicos[total].IdTecnico == Id) {
                tecnicos[total].trabalhos++;
                encontrado = 1;
            }

            total++; // só incrementa se a linha for válida
                   } else {
