...
Ativo CriarEquipamento() {
    ...
    //Logs
    char log[200];
    sprintf(log, "Equipamento criado | ID: %d | Nome: %s | Localização: %s | Estado: %s",
            a.IdAtv, a.nome, a.localizacao, EstadoAtivoToStr(a.estado));
    RegistarLog(log);
...
void EliminarAtivos() {
    ...
    //Logs
    char log[200];
    sprintf(log, "Equipamento eliminado | ID: %d | Estado: %s",id,EstadoAtivoToStr(Inativo));
    RegistarLog(log);
...
int AtualizarEstadoAtivo(int idAtivo, EstadoEquipamento novoEstado) {
    ...
    for (int i = 0; i < total; i++) {
        if (ativos[i].IdAtv == idAtivo) {
            ...
            //Logs
            char log[200];
            sprintf(log, "Equipamento atualizado | ID: %d| Estado antigo: %s | Estado Novo: %s ",
                    idAtivo, EstadoAtivoToStr(ativos[i].estado), EstadoAtivoToStr(novoEstado));
            RegistarLog(log);
...
void AdicionarReparo(int Id) {
    ...
    //Logs
    char log[200];
    sprintf(log, "Reparo Adicionado a Equipamento | ID: %d ",Id);
    RegistarLog(log);
