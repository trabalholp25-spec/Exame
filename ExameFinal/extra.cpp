...
int ProximoIdDept() {
    ...
    int maxId = 0;
    int id;
    char nome[50], resp[50], email[50], telf[50], est[50];

    while (fscanf(f, "%d;%49[^;];%49[^;];%49[^;];%49[^;];%15[^;\n];%15[^;\n];\n",&id, nome, resp, email, telf, est) == 7)
    {
        if (id > maxId)
            maxId = id;
    }
...
int ProximoIdManutencao() {
    ...
    int maxId = 0;
    int id;
    char nome[50], resp[50], email[50], telf[50], est[50], tipo[50];

    while (fscanf(f, "%d;%49[^;];%49[^;];%49[^;];%49[^;];%15[^;\n];%49[^;\n];%15[^;\n];\n",&id, nome, resp, email, telf,tipo, est) == 8)
    {
        if (id > maxId)
            maxId = id;
    }
