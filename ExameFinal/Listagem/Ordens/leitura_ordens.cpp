...
int ListarOrdensNaoConcluidas() {
    ...
    char bufferCategoria[50], bufferEstado[50], bufferTipo[50];
    char linha[256];
    int encontrou = 0; // flag para saber se encontrou alguma ordem

    while (fgets(linha, sizeof(linha), f)) {

        // Ler a linha e separar campos
        if (sscanf(linha,
                   "%d;%d;%d;%11[^;];%49[^;];%49[^;];%49[^;];%f;",
                   &m.IdOrdem,
                   &m.IdAtivo,
            ...
            // Verifica se a ordem está em Decorrer ou Iniciado
            if (m.estado == Decorrer || m.estado == Iniciado) {
                printf(
                    "\nID da Ordem: %d | Id do Ativo: %d | Id do Departamento: %d | Data do Pedido: %s | Estado: %s | Tipo: %s | Custo: %.2f€",
                    m.IdOrdem,
                    m.IdAtivo,
