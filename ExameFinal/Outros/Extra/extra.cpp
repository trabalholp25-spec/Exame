...
// Função para gravar a data de conclusão de uma ordem
int EscreverDataConclusao(int idOrdem, const char* dataConclusao) {
    FILE *f = fopen("../Dados/ordens_fim.txt", "a"); // abre em modo append
    if (!f) {
        printf("Erro ao abrir ficheiro de datas de conclusão.\n");
        return 0;
    }

    // Grava no formato: IdOrdem;DataConclusao
    fprintf(f, "%d;%s;\n", idOrdem, dataConclusao);
    fclose(f);
